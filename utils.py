...
def loadData(name): ## customize data and return data label and class_name
    ...
    if name == 'Tangdaowan':
        ...
        Tr_gt = loadmat(os.path.join(data_path, 'Tangdaowan_train.mat'))['Tangdaowan_train']
        Te_gt = loadmat(os.path.join(data_path, 'Tangdaowan_test.mat'))['Tangdaowan_test']
        class_name = ["Unassigned", "Rubber track",    "Flaggingv",    "Sandy",    "Asphalt",    "Boardwalk",    "Rocky shallows",    "Grassland",
    ...
    "Populus",    "Ulmus pumila L",    "Seawater"]
        ...
        Tr_gt = loadmat(os.path.join(data_path, 'Pingan_train.mat'))['Pingan_train']
        Te_gt = loadmat(os.path.join(data_path, 'Pingan_test.mat'))['Pingan_test']
        class_name = ["Unassigned", "Ship", "Seawater", "Trees"," Concrete structure building", "Floating pier", "Brick houses",
                      "Steel houses"," Wharf construction land", "Car", "Road"]

    elif name == 'Houston':
        data = loadmat(os.path.join(data_path, 'Houston13_HSI.mat'))['data_HS_HR']
        Tr_gt = loadmat(os.path.join(data_path, 'Houston_TrainImage.mat'))['TrainImage']
        Te_gt = loadmat(os.path.join(data_path, 'Houston_TestImage.mat'))['TestImage']
        class_name = ['Unassigned','Healthy Grass','Stressed Grass','Synthetic Grass',
        ...
        Tr_gt = loadmat(os.path.join(data_path, 'Qingyun_train.mat'))['Qingyun_train']
        Te_gt = loadmat(os.path.join(data_path, 'Qingyun_test.mat'))['Qingyun_test']
        class_name = ['Unassigned', "Trees", "Concrete building", "Car", "Ironhide building", "Plastic playground", "Asphalt road"]
    
    return data, Tr_gt, Te_gt, class_name
